...
{
	return FoxHtmlElem::create()->append(FoxHtmlElem::create('div')->classes($current->getLabelValuesClasses($form))->append(FoxHtmlElem::create('label')->append(FoxHtmlElem::create('input')->attr('type', 'radio')->attr('name', $current->getInputName())->checked($source->get('text') === $current->getInputValue())->attr('value', $source->get('text')))->append(FoxHtmlElem::create()->text($source->get('text')))->conditional($board->isValidated() && $board->isFieldInvalid($current->get('unique_id')), function ()
	{
