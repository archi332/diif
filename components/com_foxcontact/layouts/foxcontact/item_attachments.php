...
{
	return FoxHtmlElem::create('div')->append(FoxHtmlElem::create('label')->attr('for', $current->getLabelForId())->tooltip($current->get('tooltip'))->html($current->get('label'))->conditional($current->get('required'), function ()
	{
...
{
	return FoxHtmlElem::create('li')->classes('qq-upload-success')->append(FoxHtmlElem::create('span')->classes('qq-upload-file')->text($source['realname']))->append(FoxHtmlElem::create('span')->classes('qq-upload-size')->attr('style', 'display: inline-block;')->text($current->getHumanReadable($source['size'])))->append(FoxHtmlElem::create('span')->classes('qq-upload-success-text')->text(JText::_('COM_FOXCONTACT_SUCCESS')))->append(FoxHtmlElem::create('span')->classes('qq-upload-remove')->tooltip(JText::_('COM_FOXCONTACT_REMOVE_TITLE'))->text(JText::_('COM_FOXCONTACT_REMOVE_ALT'))->attr('data-file-idx', $index)->attr('data-file-url', JRoute::_("index.php?option=com_foxcontact&task=uploader.receive&uid={$uid}", false)));
}))))->show();